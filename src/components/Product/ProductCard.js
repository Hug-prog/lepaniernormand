  ...
  return (
    <div className="w-48 2xl:w-56 h-auto bg-third rounded shadow-md shadow-third mt-4 m-auto pb-2">
      ...
      <Link to={`/product/${product.id}`}>
        ...
        ></div>
        <p className="mt-4 ml-3 2xl:text-2xl">{product.libelle}</p>
        <p className="font-bold ml-3 2xl:text-2xl">{product.price}</p>
      </Link>
      ...
      <button
        ...
        className="mt-2 ml-3 bg-main text-white rounded pl-4 pr-4 pt-2 pb-2 2xl:text-md"
      >
        Ajouter au Pannier
      </button>
    </div>
